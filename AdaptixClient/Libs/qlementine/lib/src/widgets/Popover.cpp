...
void Popover::paintEvent(QPaintEvent*) {
  ...
  // Frame.
  {
    const auto frameGlobalPos = _frame->mapToGlobal(QPoint{ 0, 0 });
    const auto localPos = mapFromGlobal(frameGlobalPos);
    const auto bgRect = _frame->rect().translated(localPos);

...
{
    dlg->setWindowTitle(QCoreApplication::translate("DialogUploader", "Uploading..."));
    dlg->resize(400, 150);
    dlg->setProperty("Main", "base");

    progressBar   = new QProgressBar(dlg);
    progressBar->setRange(0, 100);
    statusLabel   = new QLabel(QCoreApplication::translate("DialogUploader", "Preparing upload..."), dlg);
    speedLabel    = new QLabel(QCoreApplication::translate("DialogUploader", "Speed: 0 KB/s"), dlg);
    cancelButton  = new QPushButton(QCoreApplication::translate("DialogUploader", "Cancel"), dlg);
    ...
    QProgressBar* progressBar, QLabel* statusLabel, QLabel* speedLabel, QPushButton* cancelButton)
{
    ...
    QObject::connect(worker, &UploaderWorker::progress, dlg, [progressBar, statusLabel](const qint64 sent, const qint64 total) {
        ...
        statusLabel->setText(QCoreApplication::translate("DialogUploader", "Uploaded %1 / %2 MB").arg(sent / (1024 * 1024)).arg(total / (1024 * 1024)));
    });

    QObject::connect(worker, &UploaderWorker::speedUpdated, dlg, [speedLabel](const double kbps) {
        speedLabel->setText(QCoreApplication::translate("DialogUploader", "Speed: %1 KB/s").arg(kbps, 0, 'f', 2));
    });

    QObject::connect(worker, &UploaderWorker::finished, dlg, [dlg, worker, progressBar, statusLabel, speedLabel]() {
        if (!worker->IsError()) {
            progressBar->setValue(100);
            statusLabel->setText(QCoreApplication::translate("DialogUploader", "Upload completed."));
    ...
    QObject::connect(worker, &UploaderWorker::failed, dlg, [dlg](const QString &msg) {
        QMessageBox::critical(dlg, QCoreApplication::translate("DialogUploader", "Upload Error"), msg);
        Q_EMIT dlg->uploadFinished(false);
        dlg->reject();
    });
...
DialogUploader::DialogUploader(const QUrl &uploadUrl, const QString &otp, const QString &filePath, QWidget *parent) : QDialog(parent)
{
    ...
    statusLabel->setText(QCoreApplication::translate("DialogUploader", "Uploading: %1 (%2 MB)").arg(fi.fileName()).arg(fi.size() / (1024 * 1024)));
